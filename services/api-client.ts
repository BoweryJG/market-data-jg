...
// API Gateway specific response structure
interface ApiGatewayResponse<T = unknown> {
  success: boolean;
  data?: T;
  status?: number;
  error?: {
...
class ApiGateway {
  ...
  // GET request
  async get<T = unknown>(url: string, options?: AxiosRequestConfig): Promise<ApiGatewayResponse<T>> {
    try {
  ...
  // POST request
  async post<T = unknown>(url: string, data?: unknown, options?: AxiosRequestConfig): Promise<ApiGatewayResponse<T>> {
    try {
  ...
  // PUT request
  async put<T = unknown>(url: string, data?: unknown, options?: AxiosRequestConfig): Promise<ApiGatewayResponse<T>> {
    try {
  ...
  // DELETE request
  async delete<T = unknown>(url: string, options?: AxiosRequestConfig): Promise<ApiGatewayResponse<T>> {
    try {
...
export class MarketInsightsApiClient {
  ...
  private async handleResponse<T>(responsePromise: Promise<ApiGatewayResponse<T>>, errorMessagePrefix: string): Promise<T> {
  ...
  public async checkHealth(): Promise<{status: string; timestamp: string}> {
    return this.handleResponse(this.apiGateway.get<{status: string; timestamp: string}>('/api/health'), 'Health check failed');
  }

  public async getCategories(): Promise<CategoryHierarchy[]> {
    return this.handleResponse(this.apiGateway.get<CategoryHierarchy[]>('/api/categories'), 'Failed to get categories');
  }

  public async getCategoryById(id: string): Promise<CategoryHierarchy> {
    return this.handleResponse(this.apiGateway.get<CategoryHierarchy>(`/api/categories/${id}`), `Failed to get category ${id}`);
  }

  public async getProcedures(limit: number = 20, offset: number = 0): Promise<(DentalProcedure | AestheticProcedure)[]> {
    return this.handleResponse(
      this.apiGateway.get<(DentalProcedure | AestheticProcedure)[]>('/api/procedures', { params: { limit, offset } }),
      'Failed to get procedures'
    );
  }

  public async getProcedureById(id: string): Promise<DentalProcedure | AestheticProcedure> {
    return this.handleResponse(this.apiGateway.get<DentalProcedure | AestheticProcedure>(`/api/procedures/${id}`), `Failed to get procedure ${id}`);
  }

  public async searchProcedures(query: string, filters?: Record<string, unknown>): Promise<(DentalProcedure | AestheticProcedure)[]> {
    return this.handleResponse(
      this.apiGateway.get<(DentalProcedure | AestheticProcedure)[]>('/api/procedures/search', { params: { q: query, ...filters } }),
      'Search failed'
    );
  }
  ...
  public async getNews(limit: number = 10, offset: number = 0): Promise<NewsArticle[]> {
    return this.handleResponse(
      this.apiGateway.get<NewsArticle[]>('/api/news', { params: { limit, offset } }),
      'Failed to get news'
    );
  }

  public async getNewsByCategory(categoryId: string, limit: number = 10, offset: number = 0): Promise<NewsArticle[]> {
    return this.handleResponse(
      this.apiGateway.get<NewsArticle[]>(`/api/news/category/${categoryId}`, { params: { limit, offset } }),
      `Failed to get news for category ${categoryId}`
    );
  }

  public async getNewsByProcedureCategory(procedureCategoryId: string, limit: number = 10, offset: number = 0): Promise<NewsArticle[]> {
    return this.handleResponse(
      this.apiGateway.get<NewsArticle[]>(`/api/news/procedure-category/${procedureCategoryId}`, { params: { limit, offset } }),
      `Failed to get news for procedure category ${procedureCategoryId}`
    );
  }

  public async getTopNewsByProcedureCategories(limit: number = 3): Promise<NewsArticle[]> {
    return this.handleResponse(
      this.apiGateway.get<NewsArticle[]>('/api/news/top-by-procedure-categories', { params: { limit } }),
      'Failed to get top news by procedure categories'
    );
  }

  public async executeQuery(query: string): Promise<ApiGatewayResponse<unknown>> {
